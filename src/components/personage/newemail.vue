  ...
  <div class="changemail">
    ...
    <div class="lan_Neirong">
      ...
      <div class="Mailbang01Group" @keyup="Enter($event)">
        <p><span>新邮件地址：</span> <input type="text" v-model="email"><a v-if="emailError">&nbsp;*请确认邮箱格式正确</a></p>
        <p><span>确认邮件地址：</span> <input type="text" v-model="emailsure"><a v-if="email!=emailsure">&nbsp;*请确认两次邮箱地址相同</a></p>
...
export default {
  ...
  methods:{
   ...
   sure:function () {
        var that = this
     if(this.emailError||(this.emailsure!=this.email)||(!this.verify)){
         return
     }
     let url=weUrl+'?ct=userHome&ac=changeMailCheck';
     let parmas={
       params:{
         email :this.email,
         verify:this.verify,
     ...
     this.$http.get(url,parmas)
       .then(function(data){
         if(data.body.state){
          this.$router.push('/personage/bindMail')
         }else {
